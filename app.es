...
function renderStockSearchResults(searchText, stocks = stockList) {
        ...
        .map(
          (item) => `
            <button
              ...
              data-stock-exchange="${item.exchange || "NSE"}"
...
async function updateStockSearch(query) {
  ...
  if (!trimmedQuery) {
    state.searchResults = [];
    resultsContainer.innerHTML = renderStockSearchResults("");
    bindStockSelectionEvents();
    return;
  }

  const requestId = ++stockSearchRequestId;
  resultsContainer.innerHTML = '<div class="info-box">Searching Kite stocks...</div>';
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Stock search failed with status ${response.status}`);
    }

    const payload = await response.json();
    const liveStocks = Array.isArray(payload) ? payload : payload.stocks;
    ...
    state.searchResults = liveStocks.map((stock) => ({
      symbol: stock.symbol || stock.tradingsymbol,
      name: stock.name || stock.company_name || stock.tradingsymbol,
      exchange: stock.exchange || "NSE",
      price: Number(stock.price ?? stock.last_price ?? 0),
      score: Number(stock.score ?? 0),
      risk: stock.risk || "Not available",
      reason: stock.reason || "Live result from Kite.",
      instrumentToken: stock.instrumentToken || stock.instrument_token,
    ...
    console.warn("Live Kite stock search unavailable; using local list.", error);
    state.searchResults = [];
    resultsContainer.innerHTML = renderStockSearchResults(trimmedQuery);
    bindStockSelectionEvents();
  }
}
...
function bindDynamicEvents() {
  ...
  if (stockSearch) {
    stockSearch.addEventListener("input", () => {
      updateStockSearch(stockSearch.value);
    });
...
function bindStockSelectionEvents() {
  document.querySelectorAll("[data-stock-symbol]").forEach((button) => {
    button.addEventListener("click", () => {
      const stock = [...stockList, ...state.searchResults].find(
        (item) =>
          item.symbol === button.dataset.stockSymbol &&
          (!button.dataset.stockExchange ||
            item.exchange === button.dataset.stockExchange)
      );
